...
public class ScsMapTriggerItem : ScsMapItem {
    public ScsMapTriggerItem( ScsSector sector ) : base( sector, sector.LastOffset ) {
        Valid = true;
    ...
    private void TsTriggerItem825() {
        ...
        for ( var i = 0; i < triggerActionCount; i++ ) {
            ...
            if ( action == ScsTokenHelper.StringToToken( "hud_parking" ) ) {
                OverlayName = "parking_ico";
                Overlay     = Store().Def.GetOverlay( OverlayName, ScsOverlayTypes.Map );
                if ( Overlay == null ) {
