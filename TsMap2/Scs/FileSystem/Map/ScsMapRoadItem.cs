...
public class ScsMapRoadItem : ScsMapItem {
    private const int StampBlockSize = 0x18;

    private List< PointF > _points;

    public ScsMapRoadItem( ScsSector sector ) : base( sector, sector.LastOffset ) {
        Valid = true;
    ...
    private void TsRoadItem854() {
        ...
        if ( RoadLook == null ) {
            Valid = false;
            Log.Warning( $"Could not find RoadLook: '{ScsTokenHelper.TokenToString( roadLookId )}'({MemoryHelper.ReadUInt64( Sector.Stream, fileOffset ):X}), "
                         + $"in {Path.GetFileName( Sector.FilePath )} @ {fileOffset}" );
        }
