...
public class TsDef {
    ...
    public TsMapOverlay? GetOverlay( string overlayName, ScsOverlayTypes overlayType ) {
        var           overlayEntry = new ScsOverlayEntry();
        TsMapOverlay? overlay      = overlayEntry.Get( overlayName, overlayType );

        if ( overlay != null && !Overlays.ContainsKey( overlay.Token ) )
            Overlays.Add( overlay.Token, overlay );
