...
public class ScsMapPrefabItem : ScsMapItem {
    ...
    private readonly List< ScsPrefabLook > _looks;

    public ScsMapPrefabItem( ScsSector sector ) : base( sector, sector.LastOffset ) {
        Valid  = true;
        _looks = new List< ScsPrefabLook >();
    ...
    public void AddLook( ScsPrefabLook look ) {
        _looks.Add( look );
    }

    public List< ScsPrefabLook > GetLooks() => _looks;
    ...
    private void TsPrefabItem825() {
        ...
        if ( Prefab == null ) {
            Valid = false;
            Log.Warning( $"Could not find Prefab: '{ScsTokenHelper.TokenToString( prefabId )}'({MemoryHelper.ReadUInt64( Sector.Stream, fileOffset ):X}), "
    ...
    private void TsPrefabItem855() {
        ...
        if ( Prefab == null ) {
            Valid = false;
            Log.Error( $"Could not find Prefab: '{ScsTokenHelper.TokenToString( prefabId )}'({prefabId:X}), item uid: 0x{Uid:X}, "
    ...
    public void UpdateLook() {
        ScsNode    originNode = Store().Map.GetNodeByUid( Nodes[ 0 ] );
        MapPalette palette    = Store().Settings.MapColor.ToBrushPalette();
        ...
        for ( var i = 0; i < Prefab.MapPoints.Count; i++ ) {
            ...
            if ( mapPoint.LaneCount == -1 ) // non-road Prefab
            {
                ...
                var prefabLook = new ScsPrefabPolyLook( polyPoints.Values.ToList() ) {
            ...
            foreach ( int neighbourPointIndex in mapPoint.Neighbours ) // TODO: Fix connection between road segments
            {
                ...
                ScsPrefabLook prefabLook = new ScsPrefabPolyLook( cornerCoords ) {
