...
namespace TsMap {
    public class TsMapper {
        ...
        private void ParseGameEntry() {
            string gameCode = null;
            string version  = null;

            // Game code
            UberFile file = UberFileSystem.Instance.GetFile( ScsPath.Def.Ets2LogoScene );
            ...
            foreach ( string line in lines ) {
                ...
                if ( key == "scene_model_animation" ) {
                    ...
                    gameCode = matches[ 0 ].Groups[ 1 ].Value;
            ...
            // Game version
            UberFile versionFile = UberFileSystem.Instance.GetFile( ScsPath.GameVersion );

            if ( versionFile != null ) {
                byte[] versionStream = versionFile.Entry.Read();
                version = Encoding.UTF8.GetString( versionStream ).Split( '\n' )[ 0 ];
            }

            Store.Instance.Game = new TsGame( gameCode, version );
        }
        ...
        public TsRoadLook LookupRoadLook( ulong lookId ) {
            return _roadLookup.ContainsKey( lookId )
                       ? _roadLookup[ lookId ]
                       : null;
        }

        public TsPrefab LookupPrefab( ulong prefabId ) {
            return _prefabLookup.ContainsKey( prefabId )
                       ? _prefabLookup[ prefabId ]
                       : null;
        }

        public TsCity LookupCity( ulong cityId ) {
            return _citiesLookup.ContainsKey( cityId )
